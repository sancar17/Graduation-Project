...
def _train_epoch_pure_torch(loader, device, model, optimizer):
    ...
    for i, input_data in enumerate(tqdm.tqdm(loader, desc="train")):
        ...
        if i % 1000 == 0 and i > 0:
            f = open("gcn_train_epoch_summary.txt", "a")
            f.write(str(loss_to_print / 1000) + "\n")
            f.close()
            logging.info("train_loss %s", str(loss_to_print / 1000))
            loss_to_print = 0
...
def _val_pure_torch(loader, device, model):
    ...
    for input_data in tqdm.tqdm(loader, desc="val"):
        ...
        f = open("gcn_val_epoch_summary.txt", "a")
...
def train_ignite(device, epochs, loss, optimizer, train_loader, val_loader, train_model):
    ...
    def log_epoch_summary(engine: Engine):
        ...
        f = open("unet_train_epoch_summary.txt", "a")
        f.write(str(train_avg_loss) + "\n")
        f.close()
        f = open("unet_val_epoch_summary.txt", "a")
        f.write(str(val_avg_loss) + "\n")
        f.close()

        with open("best_val.txt", "r") as f:
            best_val = float(f.read().strip())

        #best modeli kaydet 
        
        if best_val>val_avg_loss:
            best_val = val_avg_loss
            with open("best_val.txt", "w") as f:
                f.write("%.6f\n" % best_val)
            logging.info("aaa")
            torch.save(train_model.state_dict(), "bestunet.pt")
    ...
    def a(x):
        print(x.state.metrics)

    to_save = {"train_model": train_model, "optimizer": optimizer}
    #checkpoint_handler = Checkpoint(to_save, DiskSaver(checkpoints_dir, create_dir=True, require_empty=False), n_saved=1, score_name="val_loss", global_step_transform=global_step_from_engine(trainer))
...
def create_parser():
    ...
    parser.add_argument("-c", "--competitions", nargs="+", help="<Required> Set flag", default=["temporal"])
    return parser
...
def main(args):
    ...
    full_handler = logging.FileHandler(model_str + "_log.txt")
