...
from baselines.buildingblocks_attention import DoubleConv, ExtResNetBlock, create_decoders, create_encoders
...
def run_model(
    ...
    train_model = train_model().cuda(device_ids[0])
    train_model = DDP(train_model, device_ids)
...
def create_parser():
    ...
    parser.add_argument("--batch_size", type=int, default=1, required=True, help="Batch Size for training and validation.")
    ...
    parser.add_argument("--distributed", default=False, required=False)
...
def main(args):
    ...
    _, device = run_model(
        train_model=model, device = device, random_seed=random_seed, dataset=dataset, epochs=epochs, dataloader_config=dataloader_config, device_ids = device_ids, optimizer_config=optimizer_config, train_fraction=train_fraction, val_fraction=val_fraction, batch_size=batch_size, num_workers=num_workers, limit=limit, log_file_name=log_file_name, data_parallel = data_parallel, distributed=distributed, local_rank=local_rank)
