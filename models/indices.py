...
def run_model(
    ...
    np.random.shuffle(indices[0:43200])
    np.random.shuffle(indices[43200:43200*2])
    np.random.shuffle(indices[43200*2:43200*3])
    np.random.shuffle(indices[43200*3:43200*4])

    train_items = num_train_items

    train_indices[0:int(train_items/4)] = indices[0:int(train_items/4)]
    train_indices[int(train_items/4)+1:int(train_items/4)*2] = indices[43200:43200+int(train_items/4)]
    train_indices[int(train_items/4)*2+1:int(train_items/4)*3] = indices[2*43200:2*43200+int(train_items/4)]
    train_indices[int(train_items/4)*3+1:] = indices[3*43200:3*43200+int(train_items/4)]
...
def train_pure_torch(device, epochs, optimizer, train_loader, val_loader, train_model, log_file_name):
    ...
    for epoch in range(epochs):
        _train_epoch_pure_torch(train_loader, device, train_model, optimizer, log_file_name, mean)
...
def _train_epoch_pure_torch(loader, device, model, optimizer, log_file_name, mean):
    loss_to_print = 0
    for i, input_data in enumerate(tqdm.tqdm(loader, desc="train")):
        if(np.count_nonzero(input_data[0])>=mean): 
...
def _val_pure_torch(loader, device, model, log_file_name, epoch):
    ...
    if (epoch == 0):
        ...
        write_data_to_h5(ground_truth, "val_gt_" + str(log_file_name) + ".h5")
