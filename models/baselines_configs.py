...
from baselines.transform3D import UNetTransfomer3D
...
configs = {
    "unet": {
        ...
        "model_config": {"in_channels": 12 * 8, "n_classes": 6 * 8, "depth": 3, "wf": 6, "padding": True, "up_mode": "upsample", "batch_norm": True},
    ...
    "UNet3D": {
        # zeropad2d the input data with 0 to ensure same size after upscaling by the network inputs [495, 436] -> [496, 448]
        "dataset_config": {"transform": partial(UNetTransfomer3D.unet_pre_transform3D, zeropad2d=None, batch_dim=False)},
        "pre_transform": partial(UNetTransfomer3D.unet_pre_transform3D, zeropad2d=None, batch_dim=True, from_numpy=True),
        "post_transform": partial(UNetTransfomer3D.unet_post_transform3D, batch_dim=True),
    },
}
