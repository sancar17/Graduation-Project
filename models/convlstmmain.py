...
from competition.submission.submissionconvlstm import package_submission
...
def _train_epoch_pure_torch(loader, device, model, optimizer, log_file_name):
    ...
    for i, input_data in enumerate(tqdm.tqdm(loader, desc="train")):
        ...
        # B, C, T, H, W
        # T, B, C, H, W
        input_data = input_data.permute(2, 0, 1, 3, 4)
    ...
    f = open("ConvLSTM_train_epoch_summary_" + log_file_name +".txt", "a")
...
def main(args):
    ...
    n_gate_layers = 2
    kernel_size = 3
    hidden_dim = [feature_size for x in range(n_gate_layers-1)]
    hidden_dim.append(out_channels)
    model = ConvLSTM(input_dim=in_channels,
                    hidden_dim=hidden_dim,
                    kernel_size=(kernel_size, kernel_size),
                    num_layers=n_gate_layers,
                    batch_first=False,
                    bias=True,
